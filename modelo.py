...
class Conectar():
    ...
    def ListarAlbumes(self):
        if self.conexion.is_connected():
            try:
                ...
                senteciaSQL = "SELECT cod_album, album.nombre, interprete.nombre, interprete.apellido, genero.nombre, discografica.nombre, precio, cantidad, formato.tipo FROM album, interprete, discografica,formato,genero WHERE album.id_interprete = interprete.id_interprete AND album.id_discografica = discografica.id_discografica AND album.id_formato = formato.id_formato AND album.id_genero = genero.id_genero ORDER By album.nombre asc"
    ...
    def ListarAlbum(self,cod_album):
        if self.conexion.is_connected():
            try:
                cursor = self.conexion.cursor()
    ...
    def BorrarAlbum(self,cod_album):
        if self.conexion.is_connected():
            try:
                cursor = self.conexion.cursor()
                senteciaSQL = "DELETE FROM album WHERE cod_album="+str(cod_album)
                data=(cod_album)
                cursor.execute(senteciaSQL)
